...
\begin{columns}
    ...
    \block{Application to cosmology $\mathcal P_\mathcal R(k)$}{%
        ...
        \begin{minipage}{0.16\textwidth}
            ...
            \coloredbox{
                \textbf{The $\bm{\mathcal L}$-means algorithm}\centering\\
        ...
        \end{minipage}\\
        \coloredbox{K-nearest-neighbours and DBSCAN fail to separate the central knot into two clusters, while X-means does so reliably. $\mathcal L$-means was able to separate the central knot into multiple clusters, but also tended to over-cluster. Functional posterior plots created using \texttt{fgivenx} \cite{fgivenx}, and scatterplots showing clusters were created using a development branch of \texttt{anesthetic}\cite{anesthetic}.}
    }
    ...
    \block{Classes of Nested Sampler}{
        ...
        \begin{minipage}{0.22\textwidth}
            \innerblock{\rule[-0.6ex]{0pt}{2.5ex}Chain-based}{
                \begin{wrapfigure}[6]{r}{0.38\textwidth}
                    \includegraphics[width=0.38\textwidth]{images/slice}
                \end{wrapfigure}
                Taking a sufficiently long random walk within the likelihood contour from an existing live point will generate a new live point. For example, \polychord{} first uses the covariance matrix of the live points to whiten the space, then performs Neal's slice sampling along orthogonal directions in that space \cite{polychorda, polychordb}. \texttt{GGNS} (gradient-guided nested sampling) also implements both Neal's and Hamiltonian slice sampling, along with uniform sampling and random walks \cite{ggns}.
        ...
        \begin{minipage}{0.22\textwidth}
            \innerblock{\rule[-0.6ex]{0pt}{2.5ex}Region-based}{
                \begin{wrapfigure}[6]{r}{0.38\textwidth}
                    \includegraphics[width=0.38\textwidth]{images/region}
                \end{wrapfigure}

                Region-based samplers construct regions around the live points to approximate the likelihood contour, for example \texttt{MultiNest} constructs a series of ellipsoids \cite{multinesta, multinestb, multinestc}. These regions are usually expanded by some numerical factor to improve their chances of fully enclosing the likelihood contour, then a point is rejection-sampled from them.
