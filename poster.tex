...
\begin{columns}
    ...
    \block{Clustering choice} {%
        \begin{minipage}{0.22\textwidth}
            \coloredbox{
                ...
                2D demonstation of \polychord{} K-Nearest Neighbours, \texttt{pyclustering} X-means and \texttt{scikit-learn} (H)DBSCAN \cite{polychorda, polychordb, pyclustering, sklearn}.
            }
        \end{minipage}
        \hfill
        \begin{minipage}{0.22\textwidth}
            The nested sampling algorithm \polychord{} was originally advertised suggesting that the user should experiment with their favourite cluster-identifying methods, but provides no guidance on how to do so. We have added an interface which allows the user to substitute any clustering strategy at the Python level, allowing for easier experimentation with alternatives such as the selection provided by \texttt{scikit-learn}.

            Some algorithms are better suited than others to identifying posterior modes of nested sampling live points, for example K-means and spectral clustering need to be told the number of clusters to look for, and others may not assign every point to a cluster.
        \end{minipage}\\
   }
    \block{Application to cosmology $\mathcal P_\mathcal R(k)$}{%
        \begin{minipage}{0.22\textwidth}
            This investigation was initially motivated by a reconstruction of the primordial matter power spectrum $\mathcal P_\mathcal R(k)$ using flexknots \cite{flexknota, flexknotb}, and the Planck 2018 likelihoods \cite{planck2018v, planck2018viii}. Planck measured the $C_\ell$ multipole range $1\le\ell\le7000$, corresponding to $10^{-4}\le k\le10^{-0.3}$ \cite{ppsr}. Flexknots are parameterisations of 1D functions, consisting of a series of splines (in this case linear) joined at knots. The number and positions of knots are determined by the data, which can be performed by either combining several runs with fixed number of knots, or the number of knots being itself a parameter. In the former case, we noticed that with three knots only the mode with the central knot towards the left was being fully explored.
        \end{minipage}
        \hfill
        \begin{minipage}{0.22\textwidth}
            \coloredbox{
    ...
    \block{Classes of Nested Sampler}{
        ...
        \coloredbox{Hybrid methods combine the two approaches in an attempt to alleviate the dimensionality scaling of region-based methods, while reducing the number of likelihood evaluations made outside the contour \cite{multinesta, nsmethods, dynesty, nestedfit, ultranest}.}
